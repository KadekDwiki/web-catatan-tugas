...
$task = $core->getTask($_GET["id"]);

// Untuk menghandle penambahan data tugas
if (isset($_POST["submit"])) {
   $result = $core->updateTask($_POST['id'], $_POST["task"], $_POST["location"], $_POST["date"], $_POST["time"]);
               ...
               <div class="row">
                  <div class="col">
                     <div class=" mb-3">
                        ...
                        <input type="text" name="task" class="form-control" id="task" value="<?= $task["task"] ?>" placeholder="Buat tugas UAS Tool Teknologi Informasi" required>
                     </div>
                     <div class="mb-3">
                        <label for="location" class="form-label">Dimana tempatnya? (opsional)</label>
                        <input type="text" name="location" class="form-control" id="location" value="<?= $task["location"] ?>" placeholder="Dirumah aja">
                     </div>
                  </div>
                  <div class="col">
                     <div class="mb-3">
                        <label for="deadline" class="form-label">Tanggal batas akhir (opsional)</label>
                        <input type="date" name="date" class="form-control" id="deadline" value="<?= $task["date"] ?>">
                     </div>
                     <div class="mb-3">
